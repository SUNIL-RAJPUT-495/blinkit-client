...
import { IoBag, IoLocationOutline, IoSearch, IoLocationSharp, IoLocateSharp } from "react-icons/io5";
...
export const Cart = () => {
    ...
    return (
        <>
            ...
            {/* MAIN CART MODAL */}
            <Modal show={true} onHide={handleClose} dialogClassName="cart-right-modal" backdrop={isMobile ? false : true}>
                ...
                <Modal.Body className="modal-body-scrollable px-3 bg-light">
                    <div className="bg-white rounded border shadow-sm mb-3 overflow-hidden">
                        <div className="d-flex align-items-center p-3">
                            <div style={{ width: "50px" }}><img src={dileverytime} alt="delivery" className="w-100" /></div>
                            <div className="ms-3">
                                <h6 className="fw-bold m-0">Delivery in 8 minutes</h6>
                                <p className="m-0 text-muted small">Shipment of {totalItems} items</p>
                            </div>
                        </div>
                        <div className="p-2 border-top">
                            {cartItems.map((item, index) => (
                                <div key={item._id} className={`d-flex align-items-center justify-content-between p-2 ${index !== cartItems.length - 1 ? 'border-bottom' : ''}`}>
                                    <div className="d-flex align-items-center">
                                        <div className="border rounded p-1" style={{ width: "60px", height: "60px" }}>
                                            <img src={item.image[0]} className="w-100 h-100 object-fit-contain" alt="item" />
                                        </div>
                                        <div className="ms-3">
                                    ...
                                    <div className="d-flex justify-content-between align-items-center rounded bg-success px-2" style={{ width: "70px", height: "30px" }}>
                                        <button className="btn btn-sm text-white border-0 p-0" onClick={() => dispatch(removeItem(item._id))}>-</button>
                                        <span className="text-white small fw-bold">{item.quantity}</span>
                                        <button className="btn btn-sm text-white border-0 p-0" onClick={() => dispatch(addItem(item))}>+</button>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="d-flex align-items-center justify-content-between mb-3 p-2 rounded bg-light border">
                        <div className="d-flex align-items-center">
                            ...
                            <div>
                                ...
                                <div className="text-muted text-truncate small" style={{ maxWidth: "180px" }}>F-2 balaji apartments jaipur</div>
                            </div>
                        </div>
                        <button className="btn btn-sm btn-link text-success fw-bold text-decoration-none" onClick={() => setShowAddressModal(true)}>Change</button>
                    </div>

                    <Button className="w-100 py-2 border-0 d-flex justify-content-between align-items-center bg-success rounded-3" onClick={handleClick}>
                        <div className="text-start text-white">
                            <div className="fw-bold">₹{totalPrice + 2}</div>
                            <div style={{ fontSize: "10px" }}>TOTAL</div>
                        </div>
                        <div className="text-white fw-bold">Login to Proceed <GrNext className="ms-2" size={12} /></div>
                    </Button>
                </Modal.Footer>
            </Modal>

            {/* SAVED ADDRESSES MODAL */}
            <Modal show={showAddressModal} onHide={() => setShowAddressModal(false)} centered className="address-selection-modal">
                <Modal.Header closeButton className="border-0 pb-0">
                    <Modal.Title className="fw-bold fs-5">Select delivery address</Modal.Title>
                </Modal.Header>
                <Modal.Body className="p-3">
                    <div className="d-flex align-items-center p-3 mb-4 rounded border border-success border-opacity-25 bg-light" 
                         style={{ cursor: "pointer" }} onClick={() => { setAddnewAddress(true); setShowAddressModal(false); }}>
                    ...
                    <div style={{ maxHeight: "300px", overflowY: "auto" }}>
                        <div className="d-flex align-items-start p-3 mb-3 rounded border shadow-sm bg-white">
                            ...
                            <div>
                                ...
                                <div className="text-muted small mt-1">F-2 Balaji Apartment, Jaipur</div>
                            </div>
                        </div>
                    </div>
                </Modal.Body>
            ...
            <Modal show={addNewAddress} onHide={() => setAddnewAddress(false)} centered size="lg">
                <Modal.Header closeButton className="border-0 pb-2">
                    <Modal.Title className="fw-bold fs-5">Enter complete address</Modal.Title>
                </Modal.Header>
                <Modal.Body className="p-0 overflow-hidden">
                    <div className="container-fluid">
                        <div className="row">
                            {/* MAP SIDE */}
                            <div className="col-md-6 p-0 bg-light position-relative" style={{ minHeight: '450px' }}>
                                <div className="position-absolute top-0 start-0 w-100 p-3" style={{ zIndex: 10 }}>
                                    <div className="input-group shadow-sm bg-white rounded-3 border overflow-hidden">
                                        <span className="input-group-text bg-white border-0 text-success ps-3"><IoSearch size={18}/></span>
                                        <input type="text" className="form-control border-0 py-2 shadow-none small" placeholder="Search for area..." />
                                    </div>
                                </div>
                                <div className="w-100 h-100 bg-secondary-subtle overflow-hidden">
                                    {isLoaded ? (
                                        <GoogleMap mapContainerStyle={mapContainerStyle} center={mapCenter} zoom={15} onLoad={onMapLoad} options={{ disableDefaultUI: true }}>
                                            <MarkerF position={mapCenter} />
                                        </GoogleMap>
                                    ) : (
                                        <div className="h-100 d-flex align-items-center justify-content-center">Map Loading...</div>
                                    )}
                                </div>
                                <div className="p-3 bg-white border-top position-absolute bottom-0 w-100 shadow-sm">
                                    <div className="d-flex align-items-center mb-2">
                                        ...
                                        <div className="small fw-bold">Brij Mandal Colony, Jaipur</div>
                                    </div>
                                    <button className="btn btn-outline-success btn-sm w-100 rounded-pill fw-bold" onClick={handleGetCurrentLocation}>
                            ...
                            {/* FORM SIDE */}
                            <div className="col-md-6 p-4 border-start scroll-form" style={{ maxHeight: '550px', overflowY: 'auto' }}>
                                <form>
                                    <label className="text-muted fw-bold mb-2 small">SAVE ADDRESS AS *</label>
                                    <div className="d-flex gap-2 mb-4">
                                        <button type="button" className="btn btn-address-type active">🏠 Home</button>
                                        <button type="button" className="btn btn-address-type">🏢 Work</button>
                                        <button type="button" className="btn btn-address-type">📍 Other</button>
                                    </div>
                                    
                                    {['Flat / House no*', 'Floor (optional)', 'Area / Sector*', 'Landmark'].map((label, i) => (
                                        <div className="border-bottom mb-3 pb-1" key={i}>
                                            <label className="text-muted d-block" style={{fontSize: '10px'}}>{label.toUpperCase()}</label>
                                            <input type="text" className="form-control border-0 px-0 shadow-none small fw-semibold" placeholder={label} />
                                        </div>
                                    ))}

                                    <div className="p-3 rounded  border mb-4">
                                        <p className="fw-bold small mb-2">Receiver's Details</p>
                                        <input type="text" className="form-control border-0 border-bottom bg-transparent mb-2 shadow-none small" placeholder="Name *" />
                                        <input type="text" className="form-control border-0 border-bottom bg-transparent shadow-none small" placeholder="Phone *" />
                                    </div>

                                    <button type="submit" className="btn btn-success w-100 py-3 fw-bold rounded-3 shadow-sm border-0">Save & Proceed</button>
                                </form>
                            </div>
                        </div>
                    </div>
